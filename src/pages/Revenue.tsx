...
const Revenue = () => {
  ...
  const { data: metrics, isLoading: isLoadingMetrics } = useDashboardMetrics(selectedYear)
  const { data: monthlyRevenue = [] } = useMonthlyRevenue(selectedYear)
  const {
    data: quarterlyRevenue = [],
    isLoading: isLoadingQuarterly,
    error: quarterlyError,
  } = useQuarterlyRevenue(Number.parseInt(selectedYear))
      ...
      <div className="space-y-8">
        ...
        <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
          <Card className="border-sidebar-border">
            ...
            <CardContent>
              <div className="text-2xl font-bold">{isLoadingMetrics ? "Loading..." : formatCurrency(totalRevenue)}</div>
          ...
          <Card className="border-sidebar-border">
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {isLoadingMetrics ? "Loading..." : formatCurrency(averageMonthlyRevenue)}
              </div>
                ...
                {isLoadingMetrics ? (
                  ...
                  <span
                    className={`ml-1 inline-flex items-center ${
                      (metrics?.monthlyAverageChange ?? 0) >= 0 ? "text-green-600" : "text-red-600"
                    }`}
                  >
                    {(metrics?.monthlyAverageChange ?? 0) >= 0 ? "↑" : "↓"}{" "}
                    {Math.abs(metrics?.monthlyAverageChange ?? 0)}%
                  </span>
                )}
              </p>
            </CardContent>
          ...
          <Card className="border-sidebar-border">
            ...
            <CardContent>
              <div className="text-2xl font-bold">{isLoadingMetrics ? "Loading..." : formatCurrency(arpu)}</div>
              <p className="text-xs text-muted-foreground mt-2 flex items-center">
                ...
                {isLoadingMetrics ? (
                  ...
                  <span
                    className={`ml-1 inline-flex items-center ${
                      (metrics?.arpuChange ?? 0) >= 0 ? "text-green-600" : "text-red-600"
                    }`}
                  >
                    {(metrics?.arpuChange ?? 0) >= 0 ? "↑" : "↓"} {Math.abs(metrics?.arpuChange ?? 0)}%
                  </span>
                )}
              </p>
            </CardContent>
          ...
          <Card className="border-sidebar-border">
            ...
            <CardContent>
              <div className="text-2xl font-bold">{isLoadingMetrics ? "Loading..." : formatCurrency(mrr)}</div>
              <p className="text-xs text-muted-foreground mt-2 flex items-center">
                ...
                {isLoadingMetrics ? (
                  ...
                  <span
                    className={`ml-1 inline-flex items-center ${
                      (metrics?.mrrChange ?? 0) >= 0 ? "text-green-600" : "text-red-600"
                    }`}
                  >
                    {(metrics?.mrrChange ?? 0) >= 0 ? "↑" : "↓"} {Math.abs(metrics?.mrrChange ?? 0)}%
                  </span>
                )}
              </p>
            </CardContent>
                ...
                {isLoadingQuarterly ? (
                  <div className="flex items-center justify-center h-full">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <span className="text-sm text-muted-foreground">
                        Loading quarterly data for {selectedYear}...
                      </span>
                    </div>
                  </div>
                ) : quarterlyError ? (
                  <div className="flex items-center justify-center h-full">
                    <div className="text-center">
                      <p className="text-red-500 mb-4">Failed to load quarterly data for {selectedYear}</p>
                      <button
                  ...
                  <div className="flex items-center justify-center h-full">
                    <div className="text-center">
                      <p className="text-muted-foreground mb-2">No quarterly data available for {selectedYear}</p>
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={quarterlyRevenue} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                      ...
                      <XAxis dataKey="name" tick={{ fontSize: 12 }} tickLine={{ stroke: "#e0e0e0" }} />
                      <YAxis tickFormatter={formatCurrency} tick={{ fontSize: 12 }} tickLine={{ stroke: "#e0e0e0" }} />
                      <Tooltip
                      ...
                      />
                      <Legend />
                      <Bar dataKey="basic" name="Basic" fill="#8B5CF6" radius={[2, 2, 0, 0]} />
                      <Bar dataKey="pro" name="Pro" fill="#0EA5E9" radius={[2, 2, 0, 0]} />
                     <Bar dataKey="premium" name="Premium" fill="#10B981" radius={[2, 2, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                )}
              </CardContent>
