...
export default function WithdrawalsUI() {
  const { data: pendingRequests = [], isLoading: loadingPending } =
    usePendingPayout();
  const { data: processedRequests = [], isLoading: loadingHistory } =
    usePayoutHistory();

  const { mutate: rejectPayout, isPending: rejecting } = useRejectPayout();
  ...
  const handleSubmitReject = () => {
    ...
    rejectPayout(
      ...
      },
      {
        ...
        onError: (err: any) => {
          alert(err?.message || "Reject payout failed");
        },
      }
    );
  };
