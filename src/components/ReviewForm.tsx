...
export function ReviewForm({ onReviewSubmit, className }: ReviewFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`Request ${status === 2 ? 'approved' : 'rejected'} successfully`);
  ...
  return (
    <form onSubmit={handleSubmit} className={`space-y-4 animate-fade-in ${className}`}>
      <div className="flex flex-col space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          {status === 3 ? 'Required: Please provide reason for rejection' : 'Optional for approvals'}
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 pt-2">
        <Button
          ...
          className={`flex-1 border-sidebar-border ${status === 2 ? 'bg-green-600 hover:bg-green-700' : 'hover:bg-green-50 hover:text-green-700 hover:border-green-200'}`}
        ...
        <Button
          ...
          className={`flex-1 border-sidebar-border ${status === 3 ? 'bg-red-600 hover:bg-red-700' : 'hover:bg-red-50 hover:text-red-700 hover:border-red-200'}`}
